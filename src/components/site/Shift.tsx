...
export default function Shift({ c, locale }: { c: SiteContent; locale: Locale }) {
  const t = getDictionary(locale);
  const [before, brand, after] = t.shiftAnswer(c.brand_name);
  return (
        ...
        <div className="mt-16 grid gap-5 md:grid-cols-2">
          <Reveal className="rounded-3xl border border-ink/10 bg-paper-50 p-6 sm:p-8">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-ink-400">{t.before}</span>
            </div>
          ...
          <Reveal delay={120} className="on-dark relative overflow-hidden rounded-3xl bg-ink p-6 text-paper sm:p-8">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-citron">{t.now}</span>
            </div>
            ...
            <div className="mt-7 rounded-2xl border border-paper/10 bg-paper/[0.04] p-5 text-[15px] leading-relaxed text-paper/75">
              {before}
              <span className="rounded bg-citron px-1 font-semibold text-ink">{brand}</span>
              {after}
